...
export default function ProfileCertifier() {
  const {
    address,
    network,
    unsupportedChain,
  ...
  const {getCertifier} = useMineFunctions()
  ...
  useEffect(() => {
    getCertifier(account).then(rs => setCertifier(rs))
  }, [getCertifier, account])
...
const Information = ({name, cedula, email, tel, file}) => {
  ...
  return (
    <VStack  bg={'white'} w={"270px"} p={5} my={5} shadow={'lg'} alignItems={'start'} spacing={4} rounded={'md'} wrap={'wrap'}>
      ...
      <Box>
          <Heading fontSize='md' textAlign={'left'}>Certificado:</Heading>
          <Button
            colorScheme={"blue"}
            variant={"ghost"}
            size={"xs"}
