  ...
  const {getCertifier, acceptCertifier, removeCertifier, isCertifierAccepted} = useMineFunctions()
    ...
    return (
      <Tr>
        ...
        <Td>
          <HStack spacing={3}>
            <Button onClick={() => acceptCertifier(address)} size={'xs'} variant={'ghost'} colorScheme={'green'}>
              <CheckIcon/>
            </Button>
            <Button onClick={() => removeCertifier(address)} size={'xs'} variant={'ghost'} colorScheme={'red'}>
              <DeleteIcon/>
            </Button>
          </HStack>
        </Td>
